...
HRESULT CMeteor::Ready_GameObject()
{
...
_int CMeteor::Update_GameObject(const _float& fTimeDelta)
{
	Turn_To_Camera_Look();
	...
	if (vPos.y < m_fHeight)
	{
		...
		CFireKick* pFireKick = CFireKick::Create(m_pGraphicDev, vPos, 0.2f, 10, 100, 0.05f, L"Player_Bullet", 4.f);
...
void CMeteor::Render_GameObject()
{
...
const _vec3 * CMeteor::Get_Pos() const
{
	return m_pTransformCom->GetInfo(Engine::INFO_POS);
}

void CMeteor::Add_Effect(const _vec3* pPos)
{
	_vec3 vPos = *pPos;
	vPos.y += 0.5f;
...
HRESULT CMeteor::Add_Component()
{
	...
	pComponent = m_pTextureCom = dynamic_cast<Engine::CTexture*>(Engine::Clone(RESOURCE_STATIC, L"Texture_Meteor"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
void CMeteor::Animation(const _float & fTimeDelta)
{
...
void CMeteor::Update_Scale()
{
	...
	m_fHeight = vScale.y * 0.5f;
	m_pTransformCom->Set_Scale(vScale);
}

void CMeteor::Turn_To_Camera_Look()
{
...
CMeteor* CMeteor::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3& vInitialPos, const _vec3& vDir, const _float& fSpeed, const _float& fMaxFrame, const _float& fFrameSpeed, const _float& fScale)
{
